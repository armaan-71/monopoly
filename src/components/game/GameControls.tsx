...
export default function GameControls({ roomId, playerId }: GameControlsProps) {
  ...
  const isMyTurn =
    currentPlayerIndex !== -1 && turnIndex === currentPlayerIndex;
  ...
  return (
    ...
    >
      <Typography
        ...
        sx={{
          fontWeight: 800,
          color: isMyTurn ? "primary.main" : "text.disabled",
        }}
      >
        ...
        <Box sx={{ display: "flex", flexDirection: "column", gap: 1.5 }}>
          ...
          {hasRolled && (
            <Box
              sx={{
                display: "grid",
                gridTemplateColumns: canBuy ? "1fr 1fr" : "1fr",
                gap: 1.5,
              }}
            >
              {canBuy && (
        ...
        {(log || [])
          ...
          .map((entry, i) => (
            <Typography
              ...
              sx={{ py: 0.5, borderBottom: "1px solid rgba(255,255,255,0.05)" }}
            >
              {entry}
            </Typography>
          ))}
