...
import {
    ...
    Button, Box, Typography, Checkbox, Slider,
    Select, MenuItem, FormControl, InputLabel,
    Grid, Paper, Chip, useTheme
} from '@mui/material';
...
export default function TradeModal({ open, onClose, playerId, roomId }: TradeModalProps) {
    ...
    const validTargets = players.filter(p => p.id !== playerId && !p.isBankrupt);

    // Get properties
    const myProperties = BOARD_CONFIG.filter(config => properties[config.id]?.owner === playerId);
    const targetProperties = BOARD_CONFIG.filter(config => properties[config.id]?.owner === targetPlayerId);
    ...
    const renderPropertyItem = (propConfig: any, isSelected: boolean, toggle: (id: number) => void) => {
        ...
        return (
            ...
            >
                ...
                />
                <Box sx={{ flex: 1 }}>
                    <Typography variant="body2" fontWeight={500}>{propConfig.name}</Typography>
                    <Typography variant="caption" color="text.secondary">
                        {propConfig.group === 'special' ? 'Special' : `Value: $${propConfig.price}`}
                    </Typography>
                </Box>
                {state?.houses > 0 && <Chip label="Houses" color="error" size="small" sx={{ height: 20, fontSize: '0.65rem' }} />}
                {isMortgaged && <Chip label="M" color="default" size="small" sx={{ ml: 0.5, height: 20, fontSize: '0.65rem' }} />}
            </Paper>
        );
    };
        ...
        <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
                ...
                <Grid container spacing={3}>
                    ...
                    <Grid size={{ xs: 12, md: 6 }}>
                        <Paper sx={{ p: 2, height: '100%', bgcolor: 'rgba(0,255,0,0.05)' }}>
                            <Typography variant="h6" gutterBottom>You Offer</Typography>
                            ...
                            {myProperties.length === 0 ? (
                                <Typography variant="caption" color="text.secondary">No properties to trade</Typography>
                            ) : (
                                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                                    {myProperties.map(prop => renderPropertyItem(prop, offeringProps.includes(prop.id), toggleOfferProp))}
                                </Box>
                            )}
                        </Paper>
                    </Grid>
                    ...
                    <Grid size={{ xs: 12, md: 6 }}>
                        <Paper sx={{ p: 2, height: '100%', bgcolor: 'rgba(0,0,255,0.05)' }}>
                            <Typography variant="h6" gutterBottom>You Request</Typography>
                            ...
                            <FormControl fullWidth size="small" sx={{ mb: 2 }}>
                                ...
                                >
                                    {validTargets.map(p => (
                                        <MenuItem key={p.id} value={p.id}>{p.name} (${p.money})</MenuItem>
                                    ))}
                                </Select>
                            </FormControl>
                            ...
                            {target && (
                                <>
                                    <Box sx={{ mb: 2 }}>
                                        <Typography gutterBottom>Money: ${requestingMoney}</Typography>
                                        <Slider
                                    ...
                                    {targetProperties.length === 0 ? (
                                        <Typography variant="caption" color="text.secondary">No properties available</Typography>
                                    ) : (
                                        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                                            {targetProperties.map(prop => renderPropertyItem(prop, requestingProps.includes(prop.id), toggleRequestProp))}
                                        </Box>
                                    )}
                                </>
                            )}
            ...
            <DialogActions>
                ...
                <Button
                    ...
                    variant="contained"
                    disabled={!targetPlayerId || loading || (offeringProps.length === 0 && offeringMoney === 0 && requestingProps.length === 0 && requestingMoney === 0)}
                >
                    Propose Trade
                </Button>
            </DialogActions>
